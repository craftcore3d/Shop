...
───────────────────────────────────────────── */
function Recommendations({ cartProductIds }: { cartProductIds: string[] }) {
  const { addItem } = useCart();
  const recs = MOCK_PRODUCTS.filter((p) => !cartProductIds.includes(p.id)).slice(0, 4);
                ...
                <button
                  onClick={() =>
                    addItem({
                      ...
                      selectedColor: product.colors[0]?.label ?? "",
  ...
  return (
    <main className="min-h-screen bg-[#F2DEC7] px-4 py-10 sm:px-6 lg:px-8">
      <div className="mx-auto max-w-7xl">
        ...
        {/* Recommendations */}
        <Recommendations cartProductIds={cartProductIds} />
      </div>
    </main>
  );
}
